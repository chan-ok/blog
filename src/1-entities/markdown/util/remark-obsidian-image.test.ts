...
async function compileWithPlugin(markdown: string): Promise<string> {
  // TODO: 플러그인 구현 후 remarkPlugins에 추가
...
describe('Unit 테스트 - 기본 이미지 변환', () => {
   ...
   */
  it.skip('기본 Obsidian 이미지 문법을 표준 마크다운으로 변환해야 한다', async () => {
   ...
   */
  it.skip('캡션이 있는 Obsidian 이미지 문법을 변환해야 한다', async () => {
   ...
   */
  it.skip('중첩된 경로의 이미지를 올바르게 변환해야 한다', async () => {
   ...
   */
  it.skip('일반 텍스트는 변환하지 않아야 한다', async () => {
...
describe('Unit 테스트 - 여러 이미지 및 엣지 케이스', () => {
   ...
   * 기대 결과: 모든 이미지가 변환됨
   */
  it.skip('여러 개의 Obsidian 이미지를 모두 변환해야 한다', async () => {
    const markdown = `
# Title
...
`;
   ...
   *
   * 시나리오: 빈 `![[]]` 포함
   * 기대 결과: 빈 문법은 변환되지 않고 무시됨
   */
  it.skip('빈 Obsidian 이미지 문법은 무시해야 한다', async () => {
   ...
   * 기대 결과: 경로가 올바르게 변환됨
   */
  it.skip('공백이 포함된 경로를 올바르게 처리해야 한다', async () => {
   ...
   * 기대 결과: 파일명이 그대로 사용됨
   */
  it.skip('확장자가 없는 파일도 올바르게 처리해야 한다', async () => {
   ...
   */
  it.skip('특수 문자를 포함한 캡션을 올바르게 처리해야 한다', async () => {
    const markdown = '![[images/photo.png|Caption with "quotes" & <special>]]';
...
describe('Property-Based 테스트 - 임의 경로/캡션 조합', () => {
   ...
   */
  it.skip('임의의 경로에서 크래시하지 않아야 한다', async () => {
   ...
   * 기대 결과: 캡션이 올바르게 처리됨
   */
  it.skip('임의의 캡션에서 크래시하지 않아야 한다', async () => {
    const captionArb = fc.string({ minLength: 1, maxLength: 100 });
