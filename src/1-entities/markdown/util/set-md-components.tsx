...
export default function setMdxComponents(
  ...
  return {
    ...
    h6: Typography.h6,
    p: ({ children }) => (
      <p className="mb-6 leading-[1.9] indent-4">{children}</p>
    ),
    ul: ({ children }) => (
      <ul className="mdx-ul mb-6 pl-2 space-y-2">{children}</ul>
    ),
    ol: ({ children }) => (
      <ol className="mdx-ol mb-6 pl-2 space-y-2">{children}</ol>
    ),
    li: ({ children }) => (
      <li className="leading-[1.85] text-[15px]">{children}</li>
    ),
