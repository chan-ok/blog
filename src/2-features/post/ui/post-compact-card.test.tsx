 ...
 */
const pathArb = fc.array(fc.string({ minLength: 1, maxLength: 20 }), { minLength: 1, maxLength: 5 });
...
describe('PostCompactCard - Unit 테스트', () => {
  it('제목, 날짜, 썸네일이 올바르게 렌더링되어야 한다', async () => {
    ...
    // 제목 확인
    expect(screen.getByRole('heading', { name: '테스트 포스트 제목' })).toBeInTheDocument();
  ...
  it('카드 높이가 h-96이어야 한다', async () => {
    ...
    expect(article.className).toContain('h-96');
  });

  it('썸네일 높이가 h-32이어야 한다', async () => {
    ...
    // 썸네일 컨테이너 div 확인
    const thumbnailContainer = container.querySelector('.h-32');
    expect(thumbnailContainer).toBeInTheDocument();
    expect(thumbnailContainer?.className).toContain('h-32');
  });
