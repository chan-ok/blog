...
describe('OptimizedImage 컴포넌트', () => {
  ...
  describe('Property-Based 테스트', () => {
    ...
    it('모든 className 조합에서 스타일이 올바르게 적용되어야 한다', () => {
      fc.assert(
        fc.property(srcArb, classNameArb, (src, className) => {
          const { unmount } = render(
            <OptimizedImage
              src={src}
              alt="ClassName test"
          ...
          const img = screen.getByRole('img', { name: 'ClassName test' });
          if (className) {
            expect(img).toHaveAttribute('class', className);
          }
