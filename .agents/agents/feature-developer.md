...
- **Context7**: `resolve-library-id` → `query-docs` 순서로 호출. React, TanStack Router, Tailwind CSS, Zod 등 API가 불확실할 때 사용
- **Serena**: `get_symbols_overview` → `find_symbol` → `replace_symbol_body` 순서로 활용. Read/Edit보다 토큰 효율적. 기존 컴포넌트 재사용 여부 확인, 심볼 단위 편집에 활용
- **Exa**: 최신 라이브러리 트렌드, 베스트 프랙티스, 기술 블로그 검색에 활용
- **Grep.app**: 실제 프로덕션 코드에서 React/TypeScript 패턴 검색. 복잡한 컴포넌트 구현 패턴 참고에 활용
...
**ask-permission 명령 예시**: `pnpm test`, `pnpm tsc --noEmit`, `git add <file>`
